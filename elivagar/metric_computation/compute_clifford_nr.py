...
def compute_clifford_nr_for_circuit(circ_dir, noisy_dev, noiseless_dev, device_name, num_qubits, num_cdcs, num_shots, compute_actual_fidelity=False, num_trial_params=128,
    ...
    if num_cdcs == 128:
        noise_metric_dir = os.path.join(circ_dir, 'noise_metric')
    else: 
        noise_metric_dir = os.path.join(circ_dir, f'noise_metric_{num_cdcs}')        
    ...
    np.savetxt(device_noise_metric_dir + '/metric_tvd_score.txt', [clifford_fid])
    ...
    if compute_actual_fidelity:
        ...
        if not use_qiskit:
            ...
            np.savetxt(device_noise_metric_dir + '/actual_tvd_score.txt', [actual_fid])  
        else:
